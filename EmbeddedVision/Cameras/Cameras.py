...
class BaseCamera(PinholeCamera):
    ...
    def project_world_to_image(self, cloudpoint: Tensor, image=None, level=1) -> ImageTensor or DepthTensor:
        """
        :param cloudpoint: 3D point cloud in the World frame coordinate system
        ...
        :return: ImageTensor
        """
        cloud_in_camera_frame = self.project(cloudpoint)
        if image is not None:
            assert cloud_in_camera_frame.shape[1:3] == image.shape[-2:]
            b, cha, h, w = image.shape
            image_flatten = image.reshape([b, cha, w * h])  # shape c x H*W
            projectedImage = ImageTensor(torch.zeros([b, cha, self.sensor_resolution[0], self.sensor_resolution[0]]))
        else:
            cha = 1
            b = cloudpoint.shape[0]
            projectedImage = DepthTensor(torch.zeros([b, cha, self.sensor_resolution[0], self.sensor_resolution[0]]))
        ...
        for i in reversed(range(level)):
            im_size = self.sensor_resolution[0] // (2 ** i), self.sensor_resolution[1] // (2 ** i)
            c_ = c.copy()
            # Normalize the point cloud over the sensor resolution
            c_[:, :, 0] /= 2 ** i
            c_[:, :, 1] /= 2 ** i
            # Remove the point landing outside the image
            mask_out = ((c_[:, :, 0] < 0) + (c_[:, :, 0] > im_size[1]) +
                        (c_[:, :, 1] < 0) + (c_[:, :, 1] > im_size[0])) != 0
            # Sort the point by decreasing depth
            indexes = np.flip(c_[..., -1].argsort(0, 'quicksort'))
            c_[mask_out] = 0
            c_sorted = c_[indexes, :]
            if image is not None:
                sample = image_flatten[..., indexes].transpose([0, 2, 1])
            else:
                sample = c_sorted[..., 2:]
            # Transform the landing positions in accurate pixels
            c_x = np.int32(np.round(c_sorted[..., 0:1]))
            c_y = np.int32(np.round(c_sorted[..., 1:2]))
            rays = np.concatenate([c_x, c_y], axis=2)
            image_at_level = np.zeros([b, cha, im_size[1], im_size[0]], dtype=cloudpoint.dtype)
            image_at_level[:, :, c_y, c_x] = sample
            temp = F.interpolate(Tensor(image_at_level), projectedImage.shape[-2:])
            projectedImage[projectedImage == 0] = temp[projectedImage == 0]
